...
// Static export ships a single set of HTML with Croatian metadata baked in (see
// app/layout.tsx). This keeps the live document head in sync with the chosen
// locale: it rewrites the title, description and Open Graph/Twitter tags so
// JS-capable crawlers (Google) and link unfurlers reflect the active language,
// and emits hreflang alternates so each locale has a self-referential canonical.
function upsertMeta(
  attr: "name" | "property",
  key: string,
  content: string,
...
export function SeoSync() {
  ...
  }, [title, description, ogLocale, locale]);
